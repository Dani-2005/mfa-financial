...
class PagoService {
  ...
  Future<Map<String, dynamic>> fetchReciboDetalle(String codigoRecibo) async {
    final result = await DatabaseService.instance.query(
      'SELECT r.codigo_recibo, r.fecha_emision, r.monto_total_pagado, r.descripcion_concepto, '
      'r.metodo_pago, r.referencia, r.tipo_movimiento, '
  ...
  Future<Map<String, dynamic>> _fetchEstadoCuota(int cuotaId) async {
    final actual = await DatabaseService.instance.query(
      'SELECT estado, prestamo_id, monto_interes_generado, monto_capital_amortizado, monto_pagado_acumulado '
  ...
  Future<void> registrarPago({
    ...
    await DatabaseService.instance.query(
      ...
      '(codigo_recibo, cuota_id, prestamo_id, tipo_movimiento, fecha_emision, monto_total_pagado, descripcion_concepto, metodo_pago, referencia) '
      "VALUES (:codigo, :cuotaId, :prestamoId, 'Cuota_Ordinaria', :fecha, :monto, :concepto, :metodo, :referencia)",
      {
        'codigo': codigoRecibo,
        'cuotaId': cuotaId,
  ...
  Future<void> registrarPagoParcial({
    ...
    await DatabaseService.instance.query(
      'INSERT INTO recibos_pagos '
      '(codigo_recibo, cuota_id, prestamo_id, tipo_movimiento, fecha_emision, monto_total_pagado, descripcion_concepto, metodo_pago, referencia) '
      "VALUES (:codigo, :cuotaId, :prestamoId, 'Pago_Parcial', :fecha, :monto, :concepto, :metodo, :referencia)",
      {
        'codigo': codigoRecibo,
        'cuotaId': cuotaId,
        'prestamoId': prestamoId,
